...
export function parseNaturalLanguage(input: string): ParsedTodo {
  ...
  const { value: tags, remaining: afterTags, tokens: tagTokens } = parseTags(remaining);
  remaining = afterTags;
  ...
  const { value: time, remaining: afterTime, tokens: timeTokens } = parseTime(remaining);
  remaining = afterTime;
  parsedTokens.push(...timeTokens);

  // 5. 날짜 파싱
  const { value: date, remaining: afterDate, tokens: dateTokens } = parseDate(remaining);
  remaining = afterDate;
 ...
 * 파싱 신뢰도 계산
 */
function calculateConfidence(tokens: ParsedToken[], title: string, original: string): number {
  let confidence = 1.0;
...
export function getParsePreview(input: string): ParsePreview {
  ...
  return {
    ...
    priorityDisplay: parsed.priority ? formatPriorityDisplay(parsed.priority) : undefined,
    tagsDisplay: parsed.tags?.map((t) => `#${t}`),
    recurrenceDisplay: parsed.recurrence ? formatRecurrenceDisplay(parsed.recurrence) : undefined,
  };
}
